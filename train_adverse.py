...
from model_adverse import Darknet, compute_loss
...
def train(
    ...
    # Read reference dict
    with open('train_latent_dict.pickle', 'rb') as f:
    ...
    for batch_index, (images, targets, paths, _) in enumerate(train_dataloader):
        ...
        # Get reference latent vectors
        latent_ref = None
        for path in paths:
            key = path.replace("_foggy", "").replace("_lowlight", "")
            if "__" in key:
                key_split = key.split("__")
                ext = key_split[1].split(".")[-1]
                key = key_split[0] + "." + ext
            if latent_ref is None:
                latent_ref = latent_dict[key]
            else:
                latent_ref = torch.cat((latent_ref, latent_dict[key]), 0)
        

        # Mixed precision training
        with amp.autocast():
            output, project_out, project_ref = yolo_model(images, latent_ref)
