...
class Darknet(nn.Module):
    ...
    def forward(
        ...
        if not image_augment:
            return self.forward_once(x, latent_ref)
        else:
    ...
    def forward_once(
        ...
        if self.training:  # train
            project_ref = self.project(latent_ref)
            return yolo_out, project_out, project_ref
...
def compute_loss(
    ...
    BCELoss = BarlowTwinsLoss(device=targets.device)
    lbt = BCELoss(project_out, project_ref)
    loss = lbt.reshape(1)#lbox + lobj + lcls
